class Graph():
    ...
    def printGraphAsTable(self):
        print('\n'.join([''.join(['{:4}'.format(item)
              for item in row]) for row in self.graph]))
...
class ConnectedComponents():
    ...
    def connect(self, u, v):
        ...
        if indexU == -1 or indexV == -1 or indexU == indexV:
            print ('Ineligible conditions, u = ', u, ' v = ', v)
            return

        self.nodes[indexU].update(self.nodes[indexV])
        self.nodes.pop(indexV)

        # print ('Merged ', u , ' and ', v)
        # print ('Remaining nodes after merging are: ', self.nodes)
    ...
    def getMinimumWeightedEdges(self, graph, debug=False):
        ...
        # (print ('Final minimum edge is ', minimumEdge) if debug else None)
        return minimumEdge

class Solution():
    def boruvka(self, graph, debug=False):
        """
        ...
        while len(connectedComponents.nodes) > 1:
            ...
            try:
                ...
                connectedComponents.connect(
                    minimumEdgeWeightSource, minimumEdgeWeightTarget)
    ...
    def findCriticalAndPseudoCriticalEdges(self, n, edges, tests=None):
        debug = False
        debugIndex = -1
        ...
        criticalEdge = []
        pseudoCriticalEdge = []
        mstAllEdges = self.boruvka(graph, debug)
        if tests:
        ...
        for i in range(0, len(edges)):
            ...
            mstWithoutOneEdge = self.boruvka(graph, debug=(
                True if i == debugIndex else False))
...
if __name__ == "__main__":
    ...
    tests = [8, 8, 7, 7, 7, 7, 7]
    n = 5
    mst = solution.findCriticalAndPseudoCriticalEdges(n, edges, tests)
    print (mst)

    # edges = [[0, 1, 1], [1, 2, 1], [2, 3, 1], [0, 3, 1]]
    # n = 4
    # mst = solution.findCriticalAndPseudoCriticalEdges(n, edges, None)
    # print(mst)
